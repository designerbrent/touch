...
//Function to make the menus collapsable.
function menuCollapse(target_items){
  $('h3.title', $(target_items)).wrapInner("<a class='toggle-button'></a>");
  $('div.blockwrapper', $(target_items)).hide();
  $('a.toggle-button', $(target_items)).click(function(){
    $('div.blockwrapper').toggle('fast');
    return false;
  });
}


Drupal.behaviors.Touch = function(){
  menuCollapse('.closed');
